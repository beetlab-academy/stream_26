...
// - Классы и структуры
//Задачи:
//1. Написать класс опимсываюзщий дом, прикрутить пару протоколов которые вы сами придумаете, которые могут быть характерны для дома, и потом от этого дома отнаследовать пару дочерних классов например Умный дом, квартира, загородный дом и тд. Затем создать обьекты дочерних классов и сложить их в массив их общего типа, и потом в цикле повызывать функции этих обьектов, которых нету в родительском классе.
//2. Делаем срм для салона красоты.
//Есть мастера разных специальностей, услуги (мапятся на те же специальности)
//У услуги есть цена и продолжительность
//Есть сущность запись - у ней мастер, услуга и клиент. Клиент - у него имя и телефон. Также дата начала и конца.
//Надо сделать Календарь - класть записи на даты. И получать историю записей на конкретный день
//Получать по мастерам (все записи мастера за день) и по дням (все записи с мастерами за день)
//3. Инкапсуляция
